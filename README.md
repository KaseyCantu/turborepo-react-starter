# Turborepo: React & Storybook Starter 
...
- `packages/ui-library`: Core React components with Storybook and Vite
- `packages/tsconfig`: Shared `tsconfig.json`s used throughout the Turborepo
- `packages/eslint-preset`: ESLint preset
...
For `@kasey-turbo/ui-library`, the `build` command is the following:

```bash
tsup src/index.tsx --format esm,cjs --dts --external react
```

`tsup` compiles `src/index.tsx`, which exports all of the components in the design system, into both ES Modules and CommonJS formats as well as their TypeScript types. The `package.json` for `@kasey-turbo/ui-library` then instructs the consumer to select the correct format:

```json:ui-library/package.json
{
  "name": "@kasey-turbo/ui-library",
  "version": "0.0.0",
  "main": "./dist/index.js",
...
Run `yarn build` to confirm compilation is working correctly. You should see a folder `ui-library/dist` which contains the compiled output.
...
Each file inside of `ui-library/src` is a component inside our design system. For example:

```tsx:ui-library/src/Button.tsx
